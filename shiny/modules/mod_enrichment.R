...
mod_enrichment_server <- function(id, state) {
  ...
  server <- function(input, output, session) {
    ...
    enrichment_table <- reactive({
      ...
      all_ids <- unique(DATA$de$id)
      make_functional_enrichment(sel_ids, all_ids, DATA$fterms[[input$ontology]], DATA$id2name, input$fdr_limit)
    })
    ...
    observeEvent(input$enrichment_rows_selected, ignoreNULL = FALSE, {
      ...
      if(!is.null(rows_sel)) {
        ...
        ids <- fenr::get_term_features(DATA$fterms[[input$ontology]], term_id)
      } else {
        ids <- NULL
      }
      state$sel_term <- ids
...
make_functional_enrichment <- function(sel_ids, all_ids, trms, id2name, fdr_limit) {
  ...
  if(n > 1 && n <= CONFIG$max_points) {
    # fenr doesn't like integer ids
    all_ids <- as.character(all_ids)
    sel_ids <- as.character(sel_ids)
