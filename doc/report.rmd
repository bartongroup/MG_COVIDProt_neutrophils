...
```{r fig_participant_stats, fig.width=7, fig.height=8}
...
A couple of examples to demonstrate the strength of batch effect:

```{r fig_batch_examples, fig.width=8, fig.height=4}
...
Volcano plot and gene set enrichment analysis for the placebo.

```{r fig_dd_placebo, fig.width=4, fig.height=3}
tar_read(fig_volcano_dd_placebo)
DT::datatable(tb %>% filter(contrast == "placebo") %>% select(-contrast))
```

### Drug results

Volcano plot and gene set enrichment analysis for the drug

```{r fig_dd_drug, fig.width=4, fig.height=3}
tar_read(fig_volcano_dd_drug)
...
tar_read(fig_volcano_dd_drug_vs_placebo)
```

Here are the significant proteins:

```{r dd_drug_vs_placebo}
tar_load(dal_dd_drug_vs_placebo_proteins)
DT::datatable(dal_dd_drug_vs_placebo_proteins %>% select(-protein_names)) 
```



```{r fig_dd_drug_vs_placebo, fig.width=10, fig.height=8}
tar_read(fig_dd_drug_vs_placebo) 
```
...
-   [Interactive data explorer](https://shiny.compbio.dundee.ac.uk/private/marek_covid_prot/da_day29)
-   [Download DE results]()
