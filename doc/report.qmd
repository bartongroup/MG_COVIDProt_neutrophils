...
```{r}
#| label: fig-detection
#| fig-cap: Protein detection count as a function of protein (left) and sample (right).
#| fig-width: 10
#| fig-height: 4
...
![Distribution of raw (left) and normalised (right) protein intensities.](../fig/kernels.png){#fig-abundance-distribution}
...
```{r}
#| label: fig-ma-full
#| fig-cap: MA plots of the full model ~ treatment + day + batch + age_group + sex.
#| fig-width: 10
#| fig-height: 10
...
```{r}
#| label: fig-ma-batch
#| fig-cap: MA plots to show the batch effects. Contrasts between batches 3, 4 and 5 were calculated.
#| fig-width: 8
#| fig-height: 3
...
#| fig-cap: Examples of two proteins strongly affecged by batch effects.
#| fig-width: 9
#| fig-height: 5
